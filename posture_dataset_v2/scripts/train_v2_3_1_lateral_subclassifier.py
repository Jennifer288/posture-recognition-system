...
OUTPUT_ROOT = PROJECT_ROOT / "posture_dataset_v2" / "v2_3_1_candidate"
REPORT_DIR = PROJECT_ROOT / "posture_dataset_v2" / "reports" / "v2_3_1_lateral_subclassifier"
PLOT_DIR = REPORT_DIR / "plots"
MODEL_DIR = PROJECT_ROOT / "recognizer" / "models"
SUBMODEL_PATH = MODEL_DIR / "lateral_subclassifier_v2_3_1_candidate.joblib"
SUBMODEL_METADATA_PATH = MODEL_DIR / "lateral_subclassifier_v2_3_1_candidate.metadata.json"
SUBMODEL_RUNTIME_CONFIG_PATH = MODEL_DIR / "lateral_subclassifier_v2_3_1_candidate.runtime_config.json"
SUBMODEL_PROTOTYPE_PATH = MODEL_DIR / "lateral_prototype_bank_v2_3_1_candidate.json"
MODEL_BUNDLE_PATH = MODEL_DIR / "v2_3_1_candidate.model_bundle.json"
...
def main() -> int:
    ...
    write_csv(OUTPUT_ROOT / "v2_3_1_development_manifest.csv", manifest_rows)
    write_csv(REPORT_DIR / "v2_3_1_development_manifest.csv", manifest_rows)
    ...
    write_json(REPORT_DIR / "v2_3_1_artifact_manifest.json", artifact_payload)
...
def manifest(samples: list[LateralSample]) -> list[dict[str, Any]]:
    ...
    for sample in samples:
        rows.append({
            ...
            "data_role": "v2_3_1_lateral_development",
...
def fit_model(samples: list[LateralSample], method: str) -> LateralFineModel:
    ...
    return LateralFineModel(
        ...
        feature_names=list(LATERAL_FEATURE_NAMES),
        submodel_version="lateral_subclassifier_v2_3_1_candidate",
    )
...
def save_artifacts(
    ...
    runtime = {
        "model_version": "v2_3_1_candidate",
        "display_name": "V2.3.1候选（侧向链路修复，未闭卷）",
    ...
    metadata = {
        "model_version": "v2_3_1_candidate",
        "model_name": "lateral_subclassifier_v2_3_1_candidate",
        "candidate_name": "V2.3.1 lateral resolver propagation fix",
...
def lateral_gate_regression(samples: list[LateralSample]) -> list[dict[str, Any]]:
    ...
    for sample in samples:
        data = load_csv_playback(sample.path)
        session = CsvRecognitionSession(data, Recognizer(model_version="v2_3_1_candidate"))
        session.process_all()
        posture_records = [record for record in session.predictions if record.display_status == "POSTURE"]
...
def object_empty_unknown_gate_report() -> list[dict[str, Any]]:
    ...
    api = Recognizer(model_version="v2_3_1_candidate")
...
def artifact_hashes() -> dict[str, Any]:
    paths = {
        ...
        "v2_3_1_submodel": SUBMODEL_PATH,
        "v2_3_1_metadata": SUBMODEL_METADATA_PATH,
        "v2_3_1_prototype_bank": SUBMODEL_PROTOTYPE_PATH,
        "v2_3_1_runtime_config": SUBMODEL_RUNTIME_CONFIG_PATH,
        "v2_3_1_bundle": MODEL_BUNDLE_PATH,
    }
...
def write_report(
    ...
    lines = [
        ...
        "",
        "本报告只使用 SY1/SY2/XC1/CS1 development CSV；它们已经参与设计决策，不能作为未来最终闭卷数据。V2.3.1只修复门控覆盖、final resolver、fallback传播和侧向时间保持，不使用第一次external holdout调参。",
        "",
    ...
    (REPORT_DIR / "v2_3_1_candidate_report.md").write_text("\n".join(lines), encoding="utf-8")

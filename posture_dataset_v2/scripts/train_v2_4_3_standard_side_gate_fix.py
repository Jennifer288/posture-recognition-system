...
from recognizer.lateral_merged_subclassifier_v243 import (
    LABEL_TAXONOMY_VERSION_V243 as LABEL_TAXONOMY_VERSION,
    LateralMergedFineModelV243,
    save_lateral_merged_fine_model_v243,
    save_lateral_merged_prototype_bank_v243,
)
...
OUTPUT_ROOT = PROJECT_ROOT / "posture_dataset_v2" / "v2_4_3_candidate"
REPORT_DIR = PROJECT_ROOT / "posture_dataset_v2" / "reports" / "v2_4_3_standard_side_gate_fix"
PLOT_DIR = REPORT_DIR / "plots"
MODEL_DIR = PROJECT_ROOT / "recognizer" / "models"
SUBMODEL_PATH = MODEL_DIR / "lateral_merged_subclassifier_v2_4_3_candidate.joblib"
SUBMODEL_METADATA_PATH = MODEL_DIR / "lateral_merged_subclassifier_v2_4_3_candidate.metadata.json"
SUBMODEL_RUNTIME_CONFIG_PATH = MODEL_DIR / "lateral_merged_subclassifier_v2_4_3_candidate.runtime_config.json"
SUBMODEL_PROTOTYPE_PATH = MODEL_DIR / "lateral_merged_prototype_bank_v2_4_3_candidate.json"
MODEL_BUNDLE_PATH = MODEL_DIR / "v2_4_3_candidate.model_bundle.json"
...
EXCLUDED_HOLDOUT_FILES = [
    ...
    "V241H1_biaozhuncezuo1.csv",
    "V241H1_biaozhuncezuo2.csv",
    "V241H1_ceshenyikaozuo1.csv",
    "V241H1_ceshenyikaozuo2.csv",
    "V241H1_xiekuazuo1.csv",
    "V241H1_xiekuazuo2.csv",
    "V242H1_biaozhuncezuo1.csv",
    "V242H1_biaozhuncezuo2.csv",
    "V242H1_ceshenyikaozuo1.csv",
    "V242H1_ceshenyikaozuo2.csv",
    "V242H1_xiekuazuo1.csv",
    "V242H1_xiekuazuo2.csv",
]
...
def main() -> int:
    ...
    for path in [OUTPUT_ROOT / "v2_4_3_development_manifest.csv", REPORT_DIR / "v2_4_3_development_manifest.csv"]:
    ...
    write_csv(REPORT_DIR / "standard_side_gate_postmortem.csv", standard_side_gate_postmortem())
    ...
    write_csv(REPORT_DIR / "conditional_gate_rules.csv", conditional_gate_rules())
    ...
    write_csv(REPORT_DIR / "standard_side_leave_one_file_out.csv", [row for row in selected_lofo if row["batch"] == "CS1"])
    ...
    cross_leg_negative = cross_leg_negative_regression()
    non_lateral = non_lateral_false_trigger_report()
    leanback = leanback_regression_report()
    object_rows = object_empty_unknown_report()
    write_csv(REPORT_DIR / "cross_leg_negative_regression.csv", cross_leg_negative)
    write_csv(REPORT_DIR / "cross_leg_lateral_competition_analysis.csv", cross_leg_competition_analysis(samples, cross_leg_negative))
    ...
    write_csv(REPORT_DIR / "final_label_regression.csv", non_lateral + cross_leg_negative)
    ...
    write_csv(REPORT_DIR / "gate_latency_analysis.csv", gate_latency_rows(gate_runtime))

    after = artifact_hashes()
    artifact_payload = artifact_manifest(before, after)
    write_json(REPORT_DIR / "v2_4_3_artifact_manifest.json", artifact_payload)
    generate_plots(samples, selected_lofo, final_model)
    write_report(samples, selected_method, comparison_rows, selected_lofo, subtype_rows, diagonal_rows, gate_runtime, non_lateral, cross_leg_negative, leanback, object_rows, artifact_payload)
    (REPORT_DIR / "tests_report.md").write_text("# V2.4.3 Tests\n\n自动化测试将在候选生成后由主流程运行并回填最终结果。\n", encoding="utf-8")
    print(json.dumps({
        ...
        for row in csv.DictReader(handle):
            if str(row.get("final_label_changed", "")).lower() == "true":
                rows.append({
                    ...
                    "v2_4_3_fix": "normalization now requires stable lateral gate + physical evidence + temporal state",
                })
    if not rows:
        rows.append({
            ...
            "v2_4_3_fix": "preserve parent label unless lateral evidence is stable",
        })
    return rows
...
def normalization_rule_analysis() -> list[dict[str, Any]]:
    return [
        {"rule": "leanback_priority", "v2_4_3_behavior": "后靠二阶段稳定生效时保留后靠结果", "prevents": "侧向解析器覆盖后靠安全回退"},
        {"rule": "no_unconditional_parent_mapping", "v2_4_3_behavior": "父模型输出标准侧坐/侧身倚靠坐时不再直接规范化", "prevents": "非侧向文件仅因父标签名称被改为侧向坐姿"},
        {"rule": "stable_lateral_evidence_required", "v2_4_3_behavior": "需要侧向门控稳定、物理证据通过、temporal state active/hold/stable", "prevents": "无证据规范化"},
        {"rule": "uncertain_lateral_fallback", "v2_4_3_behavior": "已触发侧向但低置信度/低margin/OOD时显示侧向姿势", "prevents": "wrong_accept"},
    ]


def standard_side_gate_postmortem() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    source = PROJECT_ROOT / "posture_dataset_v2" / "reports" / "v2_4_2_lateral_merged_external_holdout_01" / "holdout_file_level_results.csv"
    if source.exists():
        with source.open("r", encoding="utf-8-sig", newline="") as handle:
            for row in csv.DictReader(handle):
                if "biaozhuncezuo" not in str(row.get("filename", "")):
                    continue
                rows.append({
                    "filename": row.get("filename"),
                    "data_role": "postmortem_replay_only",
                    "true_label": row.get("true_label") or SIDE_SITTING_OR_LEANING_LABEL,
                    "source_subtype": row.get("source_subtype") or "标准侧坐",
                    "v2_4_2_final_display_label": row.get("final_display_label") or row.get("file_level_output") or row.get("main_posture"),
                    "v2_4_2_result_type": row.get("result_type") or row.get("file_result_type"),
                    "known_root_cause": "parent remained 交叉腿靠背坐 and base lateral gate rejected before conditional side-vs-cross-leg competition evidence could stabilize",
                    "v2_4_3_change": "parent=交叉腿靠背坐 enters a conditional competition path only when local side prototype/classifier, side physical signature, and temporal confirmation agree",
                    "used_for_training": False,
                    "used_for_thresholds": False,
                })
    if not rows:
        for name in ["V242H1_biaozhuncezuo1.csv", "V242H1_biaozhuncezuo2.csv"]:
            rows.append({
                "filename": name,
                "data_role": "postmortem_replay_only",
                "true_label": SIDE_SITTING_OR_LEANING_LABEL,
                "source_subtype": "标准侧坐",
                "v2_4_2_final_display_label": "交叉腿靠背坐",
                "v2_4_2_result_type": "gate_miss",
                "known_root_cause": "parent=交叉腿靠背坐 was treated as non-lateral hard rejection; only transient lateral trigger was observed",
                "v2_4_3_change": "conditional competition, not unconditional pass",
                "used_for_training": False,
                "used_for_thresholds": False,
            })
    return rows
...
def conditional_gate_rules() -> list[dict[str, Any]]:
    return [
        {
            "priority": 1,
            "rule": "leanback_priority",
            "condition": "V2.2后靠二阶段稳定生效",
            "action": "保留后靠结果，侧向解析器不得覆盖",
            "hard_reject": True,
        },
        {
            "priority": 2,
            "rule": "cross_leg_parent_conditional_competition",
            "condition": "parent为交叉腿靠背坐，但local侧向Prototype/分类器、侧向物理特征和temporal证据共同支持侧向",
            "action": "允许进入侧向解析器；后续仍由Boundary/OOD/temporal决定最终输出",
            "hard_reject": False,
        },
        {
            "priority": 3,
            "rule": "protect_true_cross_leg",
            "condition": "RF与父Prototype稳定支持交叉腿靠背坐，侧向物理证据或temporal证据不足",
            "action": "保留父模型交叉腿靠背坐",
            "hard_reject": True,
        },
        {
            "priority": 4,
            "rule": "no_unconditional_cross_leg_pass",
            "condition": "只有parent标签命中交叉腿靠背坐，但缺少侧向Prototype/物理/temporal组合证据",
            "action": "不得进入侧向解析器",
            "hard_reject": True,
        },
        {
            "priority": 5,
            "rule": "uncertain_lateral_boundary",
            "condition": "已经进入侧向解析器但二分类、Prototype或OOD不确定",
            "action": "显示侧向姿势",
            "hard_reject": False,
        },
    ]
...
def strong_soft_hard_evidence_rules() -> list[dict[str, Any]]:
    return [
        ...
        {"tier": "strong", "evidence": "cross_leg_lateral_competition", "meaning": "parent为交叉腿靠背坐时，local侧向Prototype/分类器接受侧向坐姿，且标准侧坐物理签名成立", "gate_effect": "连续确认后允许进入侧向解析器"},
        ...
        {"tier": "hard_reject", "evidence": "physical_gate_active_area_too_low", "meaning": "活跃面积偏低，可能是过渡帧或非稳定侧向支撑", "gate_effect": "V2.4.3不再作为soft warning放行"},
        {"tier": "hard_reject", "evidence": "leanback_priority", "meaning": "后靠二阶段稳定生效", "gate_effect": "侧向解析器不得覆盖"},
        {"tier": "hard_reject", "evidence": "true_cross_leg_protection", "meaning": "parent与Prototype稳定支持交叉腿靠背坐，且侧向物理/temporal证据不足", "gate_effect": "保留交叉腿靠背坐"},
            ...
            for row in csv.DictReader(handle):
                if row.get("filename") == "V241H1_xiekuazuo1.csv":
                    rows.append({
                        ...
                        "root_cause": "parent=交叉腿靠背坐 was treated as non-lateral before lateral prototype/physical competition evidence could accumulate",
                        "v2_4_3_change": "交叉腿靠背坐 becomes a conditional competition parent: strong side evidence can enter lateral parser; true cross-leg protection remains",
                    })
    for row in physical_rows:
        if "physical_gate_front_back_support_out_of_range" in str(row.get("top_physical_reasons", "")):
            rows.append({
                ...
                "v2_4_3_change": "front/back remains diagnostic; not a single-item hard veto under strong evidence",
            })
    if not rows:
...
def prototype_count_and_weight_rows(model: LateralMergedFineModelV243, samples: list[LateralMergedSample]) -> list[dict[str, Any]]:
    rows = []
    label_counts = Counter(sample.merged_label for sample in samples)
...
def fit_model(samples: list[LateralMergedSample], method: str) -> LateralMergedFineModelV243:
    ...
    base = LateralMergedFineModelV243(
...
def predict_file(model: LateralMergedFineModelV243, sample: LateralMergedSample, method: str) -> dict[str, Any]:
...
def multiprototype_rows(model: LateralMergedFineModelV243) -> list[dict[str, Any]]:
    rows = []
    for label, protos in model.prototypes.items():
...
def feature_rows(model: LateralMergedFineModelV243, samples: list[LateralMergedSample]) -> list[dict[str, Any]]:
...
def threshold_rows(model: LateralMergedFineModelV243) -> list[dict[str, Any]]:
...
def save_artifacts(samples: list[LateralMergedSample], model: LateralMergedFineModelV243, method: str, comparison: list[dict[str, Any]], lofo: list[dict[str, Any]], subtype: list[dict[str, Any]], diagonal: list[dict[str, Any]]) -> None:
    save_lateral_merged_fine_model_v243(SUBMODEL_PATH, model)
    save_lateral_merged_prototype_bank_v243(SUBMODEL_PROTOTYPE_PATH, model)
    runtime = {
        "model_version": "v2_4_3_candidate",
        "display_name": "V2.4.3候选（标准侧坐门控修复，未闭卷）",
        ...
        "lateral_gate_rules": [
            ...
            "V2.4.3 conditional cross-leg competition: parent=交叉腿靠背坐 can enter lateral only with strong side evidence, prototype support, and temporal confirmation",
    ...
    metadata = {
        "model_version": "v2_4_3_candidate",
        "model_name": "lateral_merged_subclassifier_v2_4_3_candidate",
        "production_status": "candidate_only",
        "final_holdout_status": "not_started",
        "parent_model_version": "v2_2_candidate",
        "previous_candidates": ["v2_3_candidate", "v2_3_1_candidate", "v2_4_candidate", "v2_4_1_candidate", "v2_4_2_candidate"],
...
def lateral_gate_regression(samples: list[LateralMergedSample]) -> list[dict[str, Any]]:
    ...
    for sample in samples:
        data = load_csv_playback(sample.path)
        session = CsvRecognitionSession(data, Recognizer(model_version="v2_4_3_candidate"))
        session.process_all()
        postures = [record for record in session.predictions if record.display_status == "POSTURE"]
...
def non_lateral_false_trigger_report() -> list[dict[str, Any]]:
    ...
    for path in sorted(holdout_dir.glob("*.csv")):
        ...
        session_v24 = CsvRecognitionSession(data, Recognizer(model_version="v2_4_3_candidate"))
        ...
        rows.append({
            ...
            "v2_4_3_main_posture": session_v24.summary().get("main_posture"),
            "final_label_changed": session_v22.summary().get("main_posture") != session_v24.summary().get("main_posture"),
        })
    return rows


def cross_leg_negative_paths() -> list[Path]:
    relpaths = [
        "posture_dataset_v2/development/batch_A_raw/v2_jiaochatuikaobei1.csv",
        "posture_dataset_v2/development/batch_A_raw/v2_jiaochatuikaobei2.csv",
        "posture_dataset_v2/development/batch_A_raw/v2_jiaochatuikaobei3.csv",
        "posture_dataset_v2/development/batch_B_raw/v2_jiaochatuikaobeizuo4.csv",
        "posture_dataset_v2/development/batch_B_raw/v2_jiaochatuikaobeizuo5.csv",
        "posture_dataset_v2/external_holdout/H1_jiaochatuikaobeizuo1.csv",
        "posture_dataset_v2/external_holdout/H1_jiaochatuikaobeizuo2.csv",
        "posture_dataset_v2/external_holdout/H1_jiaochatuikaobeizuo3.csv",
        "posture_dataset_v2/external_holdout/holdout_batch_02/holdout_batch_jiaochatui1.csv",
        "posture_dataset_v2/external_holdout/holdout_batch_02/holdout_batch_jiaochatui2.csv",
    ]
    return [PROJECT_ROOT / rel for rel in relpaths if (PROJECT_ROOT / rel).exists()]


def cross_leg_negative_regression() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for path in cross_leg_negative_paths():
        data = load_csv_playback(path)
        session_v22 = CsvRecognitionSession(data, Recognizer(model_version="v2_2_candidate"))
        session_v243 = CsvRecognitionSession(data, Recognizer(model_version="v2_4_3_candidate"))
        session_v22.process_all()
        session_v243.process_all()
        postures = [record for record in session_v243.predictions if record.display_status == "POSTURE"]
        lateral_triggers = [record for record in postures if record.lateral_subclassifier_triggered]
        competition_frames = [record for record in postures if bool(record.cross_leg_lateral_competition_active)]
        override_frames = [record for record in postures if bool(record.conditional_gate_override)]
        labels = Counter(record.posture for record in postures if record.posture)
        v22_main = session_v22.summary().get("main_posture")
        v243_main = session_v243.summary().get("main_posture")
        stable_trigger_rate = len(lateral_triggers) / max(len(postures), 1)
        rows.append({
            "filename": path.name,
            "actual_path": str(path),
            "true_label": "交叉腿靠背坐",
            "data_role": "cross_leg_negative_regression",
            "frame_count": data.frame_count,
            "posture_frame_count": len(postures),
            "v2_2_main_posture": v22_main,
            "v2_4_3_main_posture": v243_main,
            "final_label_changed": v22_main != v243_main,
            "lateral_trigger_count": len(lateral_triggers),
            "lateral_trigger_rate": round(stable_trigger_rate, 6),
            "stable_lateral_false_trigger": stable_trigger_rate >= 0.20,
            "cross_leg_lateral_competition_frames": len(competition_frames),
            "conditional_gate_override_frames": len(override_frames),
            "final_lateral_override": v243_main in {SIDE_SITTING_OR_LEANING_LABEL, DIAGONAL_SITTING_LABEL, LATERAL_UNCERTAIN_LABEL} and v22_main != v243_main,
            "posture_counts": json.dumps(dict(labels), ensure_ascii=False),
            "protection_result": "pass" if v22_main == v243_main else "fail",
        })
    if not rows:
        rows.append({
            "filename": "__no_cross_leg_negative_files_found__",
            "true_label": "交叉腿靠背坐",
            "data_role": "cross_leg_negative_regression",
            "protection_result": "missing_data",
        })
    return rows


def cross_leg_competition_analysis(samples: list[LateralMergedSample], cross_leg_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for sample in samples:
        if sample.batch != "CS1":
            continue
        ...
        session.process_all()
        postures = [record for record in session.predictions if record.display_status == "POSTURE"]
        competition = [record for record in postures if record.cross_leg_lateral_competition_reason not in {None, "not_evaluated", "not_cross_leg_parent"}]
        rows.append({
            "filename": sample.filename,
            "true_label": sample.merged_label,
            "source_subtype": sample.source_subtype,
            "case_type": "standard_side_development",
            "main_posture": session.summary().get("main_posture"),
            "lateral_trigger_rate": round(sum(1 for record in postures if record.lateral_subclassifier_triggered) / max(len(postures), 1), 6),
            "competition_frame_count": len(competition),
            "conditional_override_count": sum(1 for record in postures if record.conditional_gate_override),
            "mean_lateral_support_score": round(float(np.mean([record.lateral_support_score or 0.0 for record in postures])), 6) if postures else 0.0,
            "mean_cross_leg_support_score": round(float(np.mean([record.cross_leg_support_score or 0.0 for record in postures])), 6) if postures else 0.0,
            "notes": "CS1 development already gates through direct side evidence; conditional path is primarily for natural parent=交叉腿靠背坐 standard-side cases.",
        })
    for row in cross_leg_rows:
        rows.append({
            "filename": row.get("filename"),
            "true_label": row.get("true_label"),
            "source_subtype": "交叉腿靠背坐",
            "case_type": "cross_leg_negative",
            "main_posture": row.get("v2_4_3_main_posture"),
            "lateral_trigger_rate": row.get("lateral_trigger_rate"),
            "competition_frame_count": row.get("cross_leg_lateral_competition_frames"),
            "conditional_override_count": row.get("conditional_gate_override_frames"),
            "mean_lateral_support_score": "",
            "mean_cross_leg_support_score": "",
            "notes": row.get("protection_result"),
        })
    return rows


def gate_latency_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for row in rows:
        # CsvRecognitionSession does not expose a dedicated first-lateral-delay
        # summary for this development script, so this report records trigger
        # coverage and the final output state as the runtime latency audit input.
        out.append({
            "filename": row.get("filename"),
            "merged_label": row.get("merged_label"),
            "source_subtype": row.get("source_subtype"),
            "lateral_trigger_rate": row.get("lateral_trigger_rate"),
            "main_posture": row.get("main_posture"),
            "latency_status": "accepted_for_development; full delay measured in GUI/evaluation exports",
        })
    return out
...
def object_empty_unknown_report() -> list[dict[str, Any]]:
    ...
    api = Recognizer(model_version="v2_4_3_candidate")
...
def artifact_hashes() -> dict[str, Any]:
    paths = {
        ...
        "v2_4_2_submodel": MODEL_DIR / "lateral_merged_subclassifier_v2_4_2_candidate.joblib",
        "v2_4_2_metadata": MODEL_DIR / "lateral_merged_subclassifier_v2_4_2_candidate.metadata.json",
        "v2_4_2_prototype_bank": MODEL_DIR / "lateral_merged_prototype_bank_v2_4_2_candidate.json",
        "v2_4_2_runtime_config": MODEL_DIR / "lateral_merged_subclassifier_v2_4_2_candidate.runtime_config.json",
        "v2_4_2_bundle": MODEL_DIR / "v2_4_2_candidate.model_bundle.json",
        "v2_4_3_submodel": SUBMODEL_PATH,
        "v2_4_3_metadata": SUBMODEL_METADATA_PATH,
        "v2_4_3_prototype_bank": SUBMODEL_PROTOTYPE_PATH,
        "v2_4_3_runtime_config": SUBMODEL_RUNTIME_CONFIG_PATH,
        "v2_4_3_bundle": MODEL_BUNDLE_PATH,
    }
...
def old_hashes(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in payload.items():
        if key in {"default_model", "default_model_version"}:
            ...
            kept[key] = value
        elif key.startswith("v2_4") and not key.startswith("v2_4_3"):
            kept[key] = value
    return kept


def artifact_manifest(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    return {"model_version": "v2_4_3_candidate", "production_status": "candidate_only", "default_model_unchanged": before.get("default_model") == after.get("default_model"), "default_model_version": default_model_version(), "old_artifacts_unchanged": old_hashes(before) == old_hashes(after), "before": before, "after": after, "created_at": datetime.now(timezone.utc).isoformat(), "git_commit": git_commit()}


def generate_plots(samples: list[LateralMergedSample], rows: list[dict[str, Any]], model: LateralMergedFineModelV243) -> None:
    _bar_plot(PLOT_DIR / "lofo_results.png", [row["filename"].replace(".csv", "") for row in rows], [1.0 if row["correct_accept"] else 0.0 for row in rows], "V2.4.3 LOFO correct_accept")
    x, y, _, _ = build_training_matrix(samples)
...
def _confusion_plot(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    draw = ImageDraw.Draw(img)
    draw.text((16, 12), "V2.4.3 LOFO confusion matrix", fill=(20, 20, 20))
    ox, oy = 130, 72
...
def write_report(samples: list[LateralMergedSample], method: str, comparison: list[dict[str, Any]], lofo: list[dict[str, Any]], subtype: list[dict[str, Any]], diagonal: list[dict[str, Any]], gate_runtime: list[dict[str, Any]], non_lateral: list[dict[str, Any]], cross_leg_negative: list[dict[str, Any]], leanback: list[dict[str, Any]], object_rows: list[dict[str, Any]], artifacts: dict[str, Any]) -> None:
    ...
    cross_leg_final_changes = sum(1 for row in cross_leg_negative if bool(row.get("final_label_changed")))
    cross_leg_stable_false_triggers = sum(1 for row in cross_leg_negative if bool(row.get("stable_lateral_false_trigger")))
    ...
    lines = [
        "# V2.4.3 Standard-Side Gate Fix Candidate Report",
        "",
        "V2.4.3 将 `标准侧坐` 与 `侧身倚靠坐` 合并为正式输出 `侧向坐姿`，`斜跨坐` 保持独立，`侧向姿势` 仅作为Boundary安全回退。",
        "",
        ...
        f"- Cross-leg-back negative final label changes: {cross_leg_final_changes}",
        f"- Cross-leg-back stable lateral false triggers: {cross_leg_stable_false_triggers}",
    ...
    (REPORT_DIR / "v2_4_3_candidate_report.md").write_text("\n".join(lines), encoding="utf-8")
